...
import { firmware_chipsets, qsc_release_parameters } from "./release-information.jsx"
...
<table>
  ...
  <tbody>
    {firmware_chipsets.map(({ label, description, qsc_cli_supported }) => (
      <tr key={label}>
        <td>{label}</td>
        <td>{description}</td>
        <td>{qsc_cli_supported ? "✓" : "✗"}</td>
      </tr>
    ))}
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    {qsc_release_parameters.map(({ product, release, images }) => (
      <tr key={product}>
        <td>{product}</td>
        <td>{release}</td>
        <td>{images.map((image) => <span key={image}>{image}<br/></span>)}</td>
      </tr>
    ))}
  </tbody>
</table>
