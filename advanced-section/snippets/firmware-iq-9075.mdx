...
- Export the `SECTOOLS` variable and compile the firmware builds
  (`<firmware-root>/<clone-directory>` is the
  top-level directory)
  ```bash
  export SECTOOLS=<firmware-root>/<clone-directory>/QCS9100.LE.2.0/common/sectoolsv2/ext/Linux/sectools
  export SECTOOLS_DIR=<firmware-root>/<clone-directory>/QCS9100.LE.2.0/common/sectoolsv2/ext/Linux
  ```
...
- Go to the following directory
   ```bash
   cd <firmware-root>/<clone-directory>/DSP.AT.1.0.1/dsp_proc/build/ms
   ```
...
- Go to the following directory
   ```bash
   cd <firmware-root>/<clone-directory>/AOP.HO.3.6.1/aop_proc/build/
   ```
...
- Create an integrated firmware binary from the individual components that you
  compiled
  ```bash
  cd <firmware-root>/<clone-directory>/QCS9100.LE.2.0/common/build
  python build.py --imf
  ```

- Firmware prebuilt is successful if the following zip files are generated in
  the `<firmware-root>/<clone-directory>/QCS9100.LE.2.0/common/build/ufs/bin`
  directory
