...
- Export the `SECTOOLS` variable and compile the firmware builds
  ...
  top-level directory)
  ```bash
  export SECTOOLS=<firmware-root>/<clone-directory>/QCS8300.LE.2.0/common/sectoolsv2/ext/Linux/sectools
  export SECTOOLS_DIR=<firmware-root>/<clone-directory>/QCS8300.LE.2.0/common/sectoolsv2/ext/Linux
  ```
...
- Create an integrated firmware binary from the individual components that
  you compiled
  ```bash
  cd <firmware-root>/<clone-directory>/QCS8300.LE.2.0/common/build
  python build.py --imf
  ```

- Firmware prebuilt is successful if the following zip files are generated in
  the `<firmware-root>/<clone-directory>/QCS8300.LE.2.0/common/build/ufs/bin`
  directory
