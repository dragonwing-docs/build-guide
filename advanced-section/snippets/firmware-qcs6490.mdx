...
- Export the `SECTOOLS` variable and compile the firmware builds
  (`<FIRMWARE_ROOT>/qualcomm-linux-spf-2-0_ap_standard_oem_nomodem`
...
- Firmware prebuilt is successful if the following zip files are generated in the
  `<FIRMWARE_ROOT>/qualcomm-linux-spf-2-0_ap_standard_oem_nomodem/QCM6490.LE.2.0/common/build/ufs/bin`
  directory
